...
class TestNoOpObserver:
    ...
    def test_on_data_start_noop(self):
        NoOpObserver().on_data_start("label", None, None)  # type: ignore[arg-type]

    def test_on_data_end_noop(self):
        NoOpObserver().on_data_end("label", None, None, None, cached=False)  # type: ignore[arg-type]

    def test_on_data_crash_noop(self):
        NoOpObserver().on_data_crash("label", None, None, RuntimeError("boom"))  # type: ignore[arg-type]

    def test_create_data_logger_returns_noop(self):
        logger = NoOpObserver().create_data_logger(None, None)  # type: ignore[arg-type]
        assert logger is _NOOP_LOGGER

    def test_create_data_logger_satisfies_protocol(self):
        logger = NoOpObserver().create_data_logger(None, None)  # type: ignore[arg-type]
        assert isinstance(logger, DataExecutionLoggerProtocol)
...
class TestNoOpLogger:
    ...
    def test_satisfies_protocol(self):
        assert isinstance(NoOpLogger(), DataExecutionLoggerProtocol)

    def test_record_noop(self):
        NoOpLogger().record(stdout="hello", success=True)

    def test_noop_logger_singleton_identity(self):
        # The singleton returned by create_data_logger is always the same object.
        obs = NoOpObserver()
        l1 = obs.create_data_logger(None, None)  # type: ignore[arg-type]
        l2 = obs.create_data_logger(None, None)  # type: ignore[arg-type]
        assert l1 is l2
