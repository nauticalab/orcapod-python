...
_N_DATA = 5
_SLEEP_S = 0.2
_SEQUENTIAL_MIN = _N_DATA * _SLEEP_S * 0.8   # 80 % of serial time
...
_EXPECTED = [x * 2 for x in range(_N_DATA)]  # [0, 2, 4, 6, 8]
...
_POOL_WORKERS = 2
_POOL_N_DATA = 6          # 3× the pool size → at least 3 rounds for sync
_POOL_SLEEP_S = 0.1          # per-data sleep
...
class TestSyncOrchestratorAsyncFunction:
    ...
    uses ``_run_async_sync`` (thread-pool bridge) so data still complete
    without deadlock.  No *inter-data* concurrency is expected, however.
    """
    ...
    def test_graceful_no_deadlock_with_io_bound(self):
        ...
        assert elapsed >= _SEQUENTIAL_MIN, (
            f"Sync orchestrator should process data sequentially "
...
class TestAsyncOrchestratorSyncFunction:
    ...
    (thread pool), so multiple data can run concurrently as OS threads.
    """
...
class TestAsyncOrchestratorAsyncFunction:
    ...
    coroutines) are async, so multiple data are awaited concurrently in
...
_SINGLE_SYNC_MIN = _SINGLE_N_DATA * _SINGLE_SLEEP_S * 0.8  # sync fn: serial


def _blocking_fn(x: int) -> int:
    """Sync fn: for-loop of time.sleep — holds its thread slot the whole time.

    One thread-pool slot occupied per data for the full duration.  With only
    one worker available, data are forced to run one-at-a-time.
    """
    for _ in range(5):
...
async def _cooperative_fn(x: int) -> int:
    ...
    All N data can be suspended simultaneously in the event loop regardless
    of thread pool size.
    """
...
class TestAsyncOrchestratorFunctionTypeDifference:
    ...
    one data at a time, while the async function is unaffected.  If
    ...
    def test_async_fn_concurrent_sync_fn_sequential_under_single_worker(self):
        ...
        expected = [x * 2 for x in range(_SINGLE_N_DATA)]

        # async+sync: one thread → data run one-at-a-time
        p_sync = _build_pipeline(
            _blocking_fn, n=_SINGLE_N_DATA, max_concurrency=None
        )
        ...
        # async+async: asyncio.sleep, all data concurrent regardless of pool
        p_async = _build_pipeline(
            _cooperative_fn, n=_SINGLE_N_DATA, max_concurrency=None
        )
...
class TestAsyncAsyncVsAsyncSync:
    ...
    ``_POOL_WORKERS`` thread slots available and ``_POOL_N_DATA`` data,
    ...
    def test_async_async_faster_than_async_sync_under_pool_saturation(self):
        ...
        expected = [x * 2 for x in range(_POOL_N_DATA)]

        # async+sync: for-loop sleep, thread pool capped at _POOL_WORKERS
        p_sync = _build_pipeline(
            _loop_sleep_double, n=_POOL_N_DATA, max_concurrency=None
        )
        results_sync, elapsed_sync = asyncio.run(
        ...
        p_async = _build_pipeline(
            _coro_sleep_double, n=_POOL_N_DATA, max_concurrency=None
        )
        results_async, elapsed_async = asyncio.run(
