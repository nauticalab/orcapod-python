"""
Tests for DataFunctionWrapper and CachedDataFunction using
InMemoryArrowDatabase as the backing store.

Covers:
- DataFunctionWrapper: full property/method delegation and protocol conformance
- CachedDataFunction construction: record_path, auto_flush default
...
- call() with inner returning None: skips record_data
- record_data: stores input hash, variation data, execution data, timestamp columns
- get_cached_output_for_data: returns None when no entry, returns data on hit
- get_cached_output_for_data: conflict resolution (multiple rows → most recent wins)
...
@pytest.fixture
def inner_pf() -> PythonDataFunction:
...
@pytest.fixture
def cached_pf(inner_pf, db) -> CachedDataFunction:
    return CachedDataFunction(inner_pf, result_database=db)
...
@pytest.fixture
def other_input_data() -> Data:
    return Data({"x": 10, "y": 20})
...
# ---------------------------------------------------------------------------
# 2. DataFunctionWrapper delegation
# ---------------------------------------------------------------------------
...
class TestWrapperDelegation:
    ...
    def test_input_data_schema_delegates(self, cached_pf, inner_pf):
        assert cached_pf.input_data_schema == inner_pf.input_data_schema

    def test_output_data_schema_delegates(self, cached_pf, inner_pf):
        assert cached_pf.output_data_schema == inner_pf.output_data_schema
...
class TestCallCacheMiss:
    def test_returns_non_none_result(self, cached_pf, input_data):
        result = cached_pf.call(input_data)
        assert result is not None

    def test_result_has_correct_value(self, cached_pf, input_data):
        result = cached_pf.call(input_data)
        assert result["result"] == 7  # 3 + 4

    def test_result_stored_in_database(self, cached_pf, input_data, db):
        cached_pf.call(input_data)
        stored = db.get_all_records(cached_pf.record_path)
        assert stored is not None
        assert stored.num_rows == 1

    def test_get_all_cached_outputs_non_empty_after_call(self, cached_pf, input_data):
        cached_pf.call(input_data)
...
class TestCallCacheHit:
    def test_second_call_returns_result(self, cached_pf, input_data):
        cached_pf.call(input_data)
        result = cached_pf.call(input_data)
        assert result is not None
        assert result["result"] == 7

    def test_second_call_does_not_add_new_record(self, cached_pf, input_data, db):
        cached_pf.call(input_data)
        cached_pf.call(input_data)
        stored = db.get_all_records(cached_pf.record_path)
        assert stored is not None
    ...
    def test_inner_function_not_called_on_cache_hit(self, inner_pf, db, input_data):
        call_count = 0
        ...
        pf = PythonDataFunction(counting_add, output_keys="result")
        cpf = CachedDataFunction(pf, result_database=db)

        cpf.call(input_data)  # cache miss — inner called once
        assert call_count == 1

        cpf.call(input_data)  # cache hit — inner should NOT be called again
        assert call_count == 1
...
class TestSkipCacheLookup:
    def test_skip_cache_lookup_still_returns_result(self, cached_pf, input_data):
        cached_pf.call(input_data)  # populate cache
        result = cached_pf.call(input_data, skip_cache_lookup=True)
        assert result is not None
        assert result["result"] == 7
...
class TestSkipCacheInsert:
    def test_skip_cache_insert_returns_result(self, cached_pf, input_data):
        result = cached_pf.call(input_data, skip_cache_insert=True)
        assert result is not None
        assert result["result"] == 7

    def test_skip_cache_insert_does_not_store(self, cached_pf, input_data, db):
        cached_pf.call(input_data, skip_cache_insert=True)
        stored = db.get_all_records(cached_pf.record_path)
        assert stored is None  # nothing stored

    def test_subsequent_call_is_still_a_cache_miss(self, cached_pf, input_data, db):
        call_count = 0
        ...
        cpf.call(input_data, skip_cache_insert=True)
        assert call_count == 1

        cpf.call(input_data)  # still a miss since nothing was stored
        assert call_count == 2


# ---------------------------------------------------------------------------
# 8. get_cached_output_for_data
# ---------------------------------------------------------------------------


class TestGetCachedOutputForData:
    def test_returns_none_before_any_call(self, cached_pf, input_data):
        result = cached_pf.get_cached_output_for_data(input_data)
        assert result is None

    def test_returns_data_after_call(self, cached_pf, input_data):
        cached_pf.call(input_data)
        result = cached_pf.get_cached_output_for_data(input_data)
        assert result is not None
        assert result["result"] == 7
    ...
    def test_different_input_returns_none(
        self, cached_pf, input_data, other_input_data
    ):
        cached_pf.call(input_data)
        result = cached_pf.get_cached_output_for_data(other_input_data)
        assert result is None
...
class TestIndependentCacheEntries:
    def test_two_inputs_stored_separately(
        self, cached_pf, input_data, other_input_data, db
    ):
        cached_pf.call(input_data)
        cached_pf.call(other_input_data)
        stored = db.get_all_records(cached_pf.record_path)
        assert stored is not None
        assert stored.num_rows == 2
    ...
    def test_each_input_retrieves_correct_result(
        ...
        result_a = cached_pf.get_cached_output_for_data(input_data)
        result_b = cached_pf.get_cached_output_for_data(other_input_data)
...
class TestRecordPathPrefixIsolation:
    def test_different_prefixes_use_different_paths(self, inner_pf, input_data):
        db = InMemoryArrowDatabase()
        cpf_a = CachedDataFunction(
            inner_pf, result_database=db, record_path_prefix=("ns", "a")
        )
        cpf_b = CachedDataFunction(
            inner_pf, result_database=db, record_path_prefix=("ns", "b")
        )

        cpf_a.call(input_data)

        # a has a cached entry; b does not
        assert cpf_a.get_cached_output_for_data(input_data) is not None
        assert cpf_b.get_cached_output_for_data(input_data) is None
...
class TestAutoFlush:
    ...
    def test_auto_flush_false_leaves_result_in_pending(self, inner_pf, input_data):
        db = InMemoryArrowDatabase()
        cpf = CachedDataFunction(inner_pf, result_database=db)
...
# ---------------------------------------------------------------------------
# 12. DataFunctionWrapper — full delegation
# ---------------------------------------------------------------------------


class TestDataFunctionWrapperDelegation:
    """DataFunctionWrapper delegates every property/method to the inner function."""

    @pytest.fixture
    def wrapper(self, inner_pf):
        return DataFunctionWrapper(inner_pf)
    ...
    def test_data_function_type_id_delegates(self, wrapper, inner_pf):
        assert wrapper.data_function_type_id == inner_pf.data_function_type_id
    ...
    def test_call_delegates(self, wrapper, input_data):
        result = wrapper.call(input_data)
        assert result is not None
        assert result["result"] == 7  # 3 + 4

    def test_async_call_delegates_through_wrapper(self, wrapper, input_data):
        result = asyncio.run(wrapper.async_call(input_data))
        assert result is not None
        assert result["result"] == 7  # 3 + 4
    ...
    def test_satisfies_data_function_protocol(self, wrapper):
        assert isinstance(wrapper, DataFunctionProtocol)


# ---------------------------------------------------------------------------
# 13. record_data — stored column structure
# ---------------------------------------------------------------------------


class TestRecordDataColumns:
    """Verify that record_data writes the expected columns into the database."""

    def test_input_data_hash_column_present(self, cached_pf, input_data, db):
        cached_pf.call(input_data)
    ...
    def test_execution_columns_present(self, cached_pf, input_data, db):
        cached_pf.call(input_data)
        table = db.get_all_records(cached_pf.record_path)
        assert table is not None
    ...
    def test_timestamp_column_present(self, cached_pf, input_data, db):
        cached_pf.call(input_data)
        table = db.get_all_records(cached_pf.record_path)
        assert table is not None
...
class TestGetAllCachedOutputsSystemColumns:
    def test_include_system_columns_true_exposes_record_id(
        self, cached_pf, input_data
    ):
        cached_pf.call(input_data)
        table = cached_pf.get_all_cached_outputs(include_system_columns=True)
        assert table is not None
        assert constants.DATA_RECORD_ID in table.column_names
    ...
    def test_include_system_columns_false_hides_record_id(
        ...
        assert table is not None
        assert constants.DATA_RECORD_ID not in table.column_names
...
class TestCallInnerReturnsNone:
    def test_inactive_inner_returns_none_and_does_not_store(
        self, inner_pf, db, input_data
    ):
        inner_pf.set_active(False)
        cpf = CachedDataFunction(inner_pf, result_database=db)
        result = cpf.call(input_data)
        assert result is None
        assert db.get_all_records(cpf.record_path) is None


# ---------------------------------------------------------------------------
# 16. get_cached_output_for_data — conflict resolution
# ---------------------------------------------------------------------------
...
class TestConflictResolution:
    ...
    def test_most_recent_wins(self, inner_pf, input_data):
        db = InMemoryArrowDatabase()
        cpf = CachedDataFunction(inner_pf, result_database=db)

        # Insert two records for the same input, with a small delay between them
        cpf.call(input_data, skip_cache_lookup=True)
        ...
        # get_cached_output_for_data should still return exactly one result
        result = cpf.get_cached_output_for_data(input_data)
        assert result is not None
        assert result["result"] == 7  # 3 + 4
...
class TestResultComputedFlag:
    ...
    def test_cache_miss_sets_computed_true(self, cached_pf, input_data):
        result = cached_pf.call(input_data)
        assert result is not None
        flag = result.get_meta_value(CachedDataFunction.RESULT_COMPUTED_FLAG)
        assert flag is True

    def test_cache_hit_sets_computed_false(self, cached_pf, input_data):
        cached_pf.call(input_data)  # first call — populates cache
        result = cached_pf.call(input_data)  # second call — cache hit
        assert result is not None
        flag = result.get_meta_value(CachedDataFunction.RESULT_COMPUTED_FLAG)
        assert flag is False

    def test_skip_cache_lookup_sets_computed_true(self, cached_pf, input_data):
        cached_pf.call(input_data)  # populate cache
        result = cached_pf.call(input_data, skip_cache_lookup=True)
        assert result is not None
        flag = result.get_meta_value(CachedDataFunction.RESULT_COMPUTED_FLAG)
    ...
    def test_skip_cache_insert_sets_computed_true(self, cached_pf, input_data):
        result = cached_pf.call(input_data, skip_cache_insert=True)
        assert result is not None
        flag = result.get_meta_value(CachedDataFunction.RESULT_COMPUTED_FLAG)
        assert flag is True
