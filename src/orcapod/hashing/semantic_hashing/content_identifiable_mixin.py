...
up a ``SemanticHasherProtocol`` manually:
...
The mixin uses the global default ``SemanticHasherProtocol`` by default, but
...
class ContentIdentifiableMixin:
    ...
    ``SemanticHasherProtocol`` to produce a stable ContentHash.
    ...
    def __init__(
        self, *, semantic_hasher: SemanticHasherProtocol | None = None, **kwargs: Any
    ) -> None:
    ...
    def _get_hasher(self) -> SemanticHasherProtocol:
        """Return the ``SemanticHasherProtocol`` to use for this object.
        ...
        Returns:
            SemanticHasherProtocol: The hasher to use.
        """
