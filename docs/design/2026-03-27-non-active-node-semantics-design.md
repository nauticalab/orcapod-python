...
Currently, `OperatorNode.iter_data()` and `OperatorNode.as_table()` both call `self.run()` unconditionally:
...
Because `run()` calls `_compute_and_store()` → `operator.process(*input_streams)` → iterates every upstream node, a single call to `iter_data()` on any `OperatorNode` cascades computation through the entire upstream DAG.

`SourceNode` demonstrates the "no computation on iteration" principle: its `iter_data()` reads from `self.stream` or `self._cached_results` directly and never calls `run()`. (Note: `SourceNode.iter_data()` raises `RuntimeError` in read-only mode rather than returning empty — the new empty-fallback behaviour for `OperatorNode` is a deliberate extension beyond what `SourceNode` provides, appropriate because an `OperatorNode` always has a known output schema from `output_schema()`.)

**Transitively affected methods in `StreamBase`:**

- `__iter__` delegates to `iter_data()`
- `flow()` calls `iter_data()` in a list comprehension; its current docstring ("This will trigger any upstream computation") will be incorrect after the fix and must be updated
...
**Note on `FunctionNode`:** `FunctionNode.iter_data()` is a generator and does not call `run()`. It calls `_ensure_iterator()` → `self._input_stream.iter_data()` on its wired input. With the old `OperatorNode` this cascaded upstream computation; after the fix, that cascade is eliminated. However, `FunctionNode.iter_data()` is **not** fully passive — it still executes its own per-data function for data not already cached in the result DB (Phase 2 processing). This is intentional and unaffected by this change. What changes is only that the upstream `OperatorNode` cascade no longer occurs. No changes to `FunctionNode` are required.

---

## 2. Design Goals

1. **Iteration is read-only.** `iter_data()`, `as_table()`, `__iter__()`, `flow()`, and all methods that delegate to them must never trigger upstream computation.
...
**Important semantics:** When `_pipeline_database is not None` and `_cache_mode == CacheMode.REPLAY`, this method **always returns non-None** (either real rows or a zero-row table). The step-3 empty fallback in `iter_data` / `as_table` is therefore never reached when REPLAY+DB is active.

This helper is intentionally **state-free**: it does not assign to `self._cached_output_stream` and does not call `_update_modified_time()`. Consequently, repeated calls to `iter_data()` or `as_table()` that reach step 2 will re-query the DB on every call — there is no in-memory caching via this path. Caching from DB results is the responsibility of the computation paths (`run()`, `execute()`).

### 3.3 Updated Methods in `OperatorNode`

**`iter_data()`** — remove `self.run()`, replace with passive read:

```
1. If self._cached_output_stream is not None → return its iter_data()
2. db_stream = _load_cached_stream_from_db()
   if db_stream is not None → return db_stream.iter_data()
...
The current `run()` checks `is_stale` and clears the cache if stale. After the fix, `iter_data()` and `as_table()` read from `_cached_output_stream` without checking `is_stale`. This is **intentional**: iteration is now strictly passive; implicit cache invalidation is a side effect that violates the read-only contract.

Consequence: if an upstream is modified after `run()` was called, the user must call `run()` again to get fresh results. Calling `iter_data()` or `as_table()` without an intermediate `run()` will silently return old cached data.
...
- **No computation called, no cache:** `iter_data()` returns an empty iterator; `as_table()` / `_make_empty_table()` returns a zero-row table with correct schema. No exception is raised. This requires `output_schema()` to succeed, which it does for all valid `OperatorNode` constructions (live nodes have `_operator` set; read-only nodes have `_stored_schema`).
- **DB errors:** any database exception from `_load_cached_stream_from_db()` propagates unchanged to the caller.
- **Stale cache:** `iter_data()` does not check `is_stale`. Users can inspect `node.is_stale` to detect whether cached data is outdated before iterating.
- **`async_iter_data`:** `OperatorNode` does not override this; it continues to raise `NotImplementedError`. Out of scope for this change.
...
- `test_iter_data` — add `node.run()` before `list(node.iter_data())`
- `test_as_table` — add `node.run()` before `node.as_table()`

**`tests/test_core/operators/test_operator_node_attach_db.py`:**
- `test_iter_data_without_database` — add `node.run()` before `list(node.iter_data())`
- `test_iter_data_with_database` — add `node.run()` before `list(node.iter_data())`
...
| Test | Description |
|------|-------------|
| `test_iter_data_without_run_returns_empty` | `SourceNode → OperatorNode`. Iterate without `run()`. Assert zero data and `_operator.process` never called (spy). |
...
| `test_iter_data_after_run_returns_results` | Call `node.run()` first, then iterate. Assert expected data returned. |
...
| Test | Description |
|------|-------------|
| `test_iter_data_replay_mode_returns_db_contents` | REPLAY mode: use a prior LOG-mode `node.run()` to populate DB, then create fresh REPLAY node (no in-memory cache). Assert `iter_data()` without `run()` returns DB records via `_load_cached_stream_from_db`. |
| `test_iter_data_replay_mode_no_records_returns_empty` | REPLAY mode: DB exists but no records yet. Assert empty. |
| `test_iter_data_no_db_no_run_returns_empty` | No DB, no `run()`. Assert empty. |
| `test_iter_data_log_mode_no_run_returns_empty` | LOG mode: first do a LOG-mode `run()` to populate DB records. Confirm records exist at `pipeline_path` (to prove the guard is actually exercised). Create a fresh node with cleared in-memory cache. Assert `iter_data()` without `run()` returns empty — not the stale DB records. |
...
- Async (`async_iter_data`) variants — follow-up work tracked separately.
